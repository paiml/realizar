...
fn test_trace_layer_by_layer() {
    ...
    eprintln!("Initial hidden: sum={:.4}, norm={:.4}", 
              hidden.iter().sum::<f32>(),
    ...
    for layer_idx in 0..model.config.num_layers {
        ...
        if layer_idx < 3 || layer_idx >= model.config.num_layers - 2 {
            eprintln!("Layer {:2}: rms={:.4}, sum={:.4}, norm={:.4}, range=[{:.2}, {:.2}]",
    ...
    let logits = model.forward_single_with_cache(token_id, &mut cache2, 0).expect("Forward failed");
