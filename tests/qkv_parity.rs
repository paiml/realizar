...
mod tests {
    use realizar::gguf::{MappedGGUFModel, OwnedQuantizedModel, OwnedQuantizedModelCuda, QuantizedGenerateConfig, OwnedQKVWeights};
    ...
    fn test_phase16_rmsnorm_trace() {
        ...
        let normed: Vec<f32> = embedding.iter()
        ...
        let qkv = match qkv_weight {
            ...
            OwnedQKVWeights::Separate { q, k, v } => {
                ...
                eprintln!("  Q L2: {:.6}", q_out.iter().map(|x| x*x).sum::<f32>().sqrt());
                eprintln!("  K L2: {:.6}", k_out.iter().map(|x| x*x).sum::<f32>().sqrt());
                eprintln!("  V L2: {:.6}", v_out.iter().map(|x| x*x).sum::<f32>().sqrt());
    ...
    fn test_phase16b_direct_qkv_gemv() {
        ...
        match CudaExecutor::new(0) {
            Ok(mut executor) => {
                ...
                executor.load_quantized_weights_with_type(&cache_key, q_data, 12)
                ...
                // Run GPU GEMV
                match executor.q4k_gemv_cached(&cache_key, &normed, &mut gpu_q, q_out_dim as u32, q_in_dim as u32) {
                    Ok(()) => {
                        ...
                        if relative_diff < 1.0 {
                            ...
                            let avg_diff: f32 = cpu_q.iter()
                                .zip(gpu_q.iter())
                                .map(|(a, b)| a - b)
                                .sum::<f32>() / cpu_q.len() as f32;
    ...
    fn test_phase16c_layer_trace() {
        ...
        eprintln!("Input: embedding after RMSNorm, L2={:.6}\n", normed.iter().map(|x| x*x).sum::<f32>().sqrt());

        // Get weight references
        let (q_data, q_in, q_out, k_data, k_in, k_out, v_data, v_in, v_out, v_qtype) = match &layer.qkv_weight {
            OwnedQKVWeights::Separate { q, k, v } => (
                &q.data, q.in_dim, q.out_dim,
                &k.data, k.in_dim, k.out_dim,
                &v.data, v.in_dim, v.out_dim, v.qtype
            ),
            _ => {
        ...
        match CudaExecutor::new(0) {
            Ok(mut executor) => {
                ...
                executor.load_quantized_weights_with_type(&q_key, q_data, 12).expect("Q upload");
                executor.load_quantized_weights_with_type(&k_key, k_data, 12).expect("K upload");
                executor.load_quantized_weights_with_type(&v_key, v_data, v_qtype).expect("V upload");

                // === Q Projection ===
                let cpu_q = fused_q4k_parallel_matvec(q_data, &normed, q_in, q_out).expect("CPU Q");
                let mut gpu_q = vec![0.0f32; q_out];
                executor.q4k_gemv_cached(&q_key, &normed, &mut gpu_q, q_out as u32, q_in as u32).expect("GPU Q");

                let q_diff = cpu_q.iter().zip(gpu_q.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f32>().sqrt();
                let q_rel = q_diff / cpu_q.iter().map(|x| x*x).sum::<f32>().sqrt() * 100.0;
                eprintln!("Q: CPU L2={:.4}, GPU L2={:.4}, diff={:.6} ({:.4}%) {}",
                    cpu_q.iter().map(|x| x*x).sum::<f32>().sqrt(),
                    gpu_q.iter().map(|x| x*x).sum::<f32>().sqrt(),
                    q_diff, q_rel, if q_rel < 1.0 { "✅" } else { "❌" });

                // === K Projection ===
                let cpu_k = fused_q4k_parallel_matvec(k_data, &normed, k_in, k_out).expect("CPU K");
                let mut gpu_k = vec![0.0f32; k_out];
                executor.q4k_gemv_cached(&k_key, &normed, &mut gpu_k, k_out as u32, k_in as u32).expect("GPU K");

                let k_diff = cpu_k.iter().zip(gpu_k.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f32>().sqrt();
                let k_rel = k_diff / cpu_k.iter().map(|x| x*x).sum::<f32>().sqrt() * 100.0;
                eprintln!("K: CPU L2={:.4}, GPU L2={:.4}, diff={:.6} ({:.4}%) {}",
                    cpu_k.iter().map(|x| x*x).sum::<f32>().sqrt(),
                    gpu_k.iter().map(|x| x*x).sum::<f32>().sqrt(),
                    k_diff, k_rel, if k_rel < 1.0 { "✅" } else { "❌" });
                ...
                let v_result = executor.q6k_gemv_cached(&v_key, &normed, &mut gpu_v, v_out as u32, v_in as u32);
                match v_result {
                    Ok(()) => {
                        let v_diff = cpu_v.iter().zip(gpu_v.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f32>().sqrt();
                        let v_rel = v_diff / cpu_v.iter().map(|x| x*x).sum::<f32>().sqrt() * 100.0;
                        eprintln!("V (Q6_K): CPU L2={:.4}, GPU L2={:.4}, diff={:.6} ({:.4}%) {}",
                            cpu_v.iter().map(|x| x*x).sum::<f32>().sqrt(),
                            gpu_v.iter().map(|x| x*x).sum::<f32>().sqrt(),
                            v_diff, v_rel, if v_rel < 1.0 { "✅" } else { "❌" });
                    }
                    Err(e) => {
                        eprintln!("V (Q6_K): GPU GEMV failed: {:?}", e);
                        eprintln!("  CPU V L2={:.4}", cpu_v.iter().map(|x| x*x).sum::<f32>().sqrt());
                    }
                }
