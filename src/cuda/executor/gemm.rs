...
mod tests {
    ...
    fn test_gemm_fused_with_bias() {
        ...
        let mut c = vec![0.0; 4];
        exec.gemm_fused(&a, &b, Some(&bias), &mut c, 2, 2, 2, 0).unwrap();
    ...
    fn test_async_copy_roundtrip() {
        ...
        // Copy to GPU async
        unsafe { exec.copy_to_gpu_async(&mut buf, &data).unwrap(); }
        exec.synchronize_transfer().unwrap();

        // Copy back from GPU async
        let mut result = vec![0.0f32; 4];
        unsafe { exec.copy_from_gpu_async(&buf, &mut result).unwrap(); }
        exec.synchronize_transfer().unwrap();
