impl CudaExecutor {
    ...
    pub fn batched_incremental_attention_into(
        ...
        // PMAT-286: Fused K+V scatter — ONE launch for both K and V.
        // Uses blockIdx.z (z=0=K, z=1=V). Saves 28 launches/step vs 2 separate launches.
        // Previous: 2 launches per layer (56 total). Now: 1 launch per layer (28 total).
        ...
        // PMAT-286: Fused K+V scatter — ONE launch, blockIdx.z selects K(0)/V(1)
        let fused_scatter_config = LaunchConfig {
            grid: (num_kv_heads as u32, m as u32, 2), // z=2 for K+V
            block: (head_dim as u32, 1, 1),
            shared_mem: 0,
        };

        let scatter_entry_name = batched_scatter_key.clone();
