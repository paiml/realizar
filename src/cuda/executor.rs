...
//!
//! This file currently exceeds the 800-line limit (~15K lines).
//! Future refactoring should split into:
//! - `executor/core.rs`: Context, streams, profiling
//! - `executor/weights.rs`: Weight loading and caching
//! - `executor/gemm.rs`: GEMM/GEMV operations
//! - `executor/attention.rs`: Attention kernels
//! - `executor/activations.rs`: SiLU, GELU, RMSNorm
//! - `executor/graph.rs`: CUDA graph capture/replay
...
mod proptests {
    ...
    fn test_tcov001ai_fused_gate_up_q4k_gemv_ptx() {
        let kernels = CudaKernels::new();
        let ptx = kernels.generate_ptx(&KernelType::FusedGateUpQ4KGemv {
            k: 4096,
            n: 11008,
        });
        assert!(ptx.contains(".version"));
    ...
    fn test_tcov002_kernel_names_non_empty() {
        ...
        let test_cases: Vec<KernelType> = vec![
            ...
            KernelType::GemmTiled { m: 64, n: 64, k: 64, tile_size: 32 },
            ...
            KernelType::Attention { seq_len: 128, head_dim: 64, causal: false },
            KernelType::RmsNorm { hidden_size: 4096, epsilon: 1e-6 },
            KernelType::ResidualAdd { n: 4096 },
        ];

        for kernel_type in test_cases {
            let name = kernels.kernel_name(&kernel_type);
            assert!(!name.is_empty(), "KernelType {:?} should have non-empty name", kernel_type);
        }
    }
    ...
    fn test_tcov007_presets_coverage() {
        ...
        match tc_attn {
            KernelType::AttentionTensorCore { seq_len, head_dim, n_heads, causal } => {
                assert_eq!(seq_len, 2048);
                assert_eq!(head_dim, 64);
                assert_eq!(n_heads, 32);
                assert!(causal);
    ...
    fn test_tcov010_more_kernel_names() {
        ...
        let kernel_types: Vec<KernelType> = vec![
            KernelType::FusedRmsNormQ4KGemv { k: 4096, n: 4096, epsilon: 1e-6 },
            ...
            KernelType::BatchedQ4KGemv { k: 4096, n: 4096, m: 4 },
            KernelType::MultiWarpBatchedQ4KGemv { k: 4096, n: 4096, warps: 4 },
            KernelType::TensorCoreQ4KGemm { m: 1, n: 4096, k: 4096 },
            ...
            KernelType::PreciseRopeNeoxIndirect { num_heads: 32, head_dim: 64, theta: 10000.0 },
            KernelType::Silu { n: 4096 },
            KernelType::Gelu { n: 4096 },
            KernelType::ElementwiseMul { n: 4096 },
            KernelType::FusedSwiglu { n: 4096 },
            KernelType::FusedQKV { hidden_size: 4096, kv_dim: 4096 },
            KernelType::FusedGateUp { hidden_size: 4096, intermediate_size: 11008 },
