...
impl ModelFixture {
    ...
    fn generate_apr_data(config: &ModelConfig) -> Vec<u8> {
        ...
        // Tensor count
        let tensor_count = 3 + config.num_layers * 6; // embed + norm + lm_head + layer weights
        data.extend_from_slice(&(tensor_count as u32).to_le_bytes());

        // Metadata offset (after header)
        data.extend_from_slice(&64u64.to_le_bytes());

        // Placeholder metadata size (will be filled)
        let metadata_size_offset = data.len();
        data.extend_from_slice(&0u32.to_le_bytes());

        // Tensor index offset (placeholder)
        ...
        // Metadata (JSON)
        let metadata = format!(
            r#"{{"architecture":"{}","hidden_dim":{},"num_layers":{},"vocab_size":{}}}"#,
            config.architecture, config.hidden_dim, config.num_layers, config.vocab_size
        );
        ...
        // Tensor index (simplified: just offsets)
        // For now, write placeholder tensor index
        let tensor_index_size = tensor_count * 32; // name_offset(8) + data_offset(8) + shape(8) + dtype(4) + pad(4)
        data.resize(data.len() + tensor_index_size, 0);
        ...
        // Add minimal tensor data (just zeros for structure)
        let tensor_data_size = config.vocab_size * config.hidden_dim * 4; // F32 embedding
        data.resize(data.len() + tensor_data_size, 0);
