...
fn test_q4_1_candle_layout_correctness() {
    ...
    assert!((result[0] - 0.0).abs() < 1e-5, "pos 0: expected 0.0, got {}", result[0]);
    assert!((result[1] - 2.0).abs() < 1e-5, "pos 1: expected 2.0, got {}", result[1]);
    assert!((result[2] - 4.0).abs() < 1e-5, "pos 2: expected 4.0, got {}", result[2]);
    assert!((result[3] - 6.0).abs() < 1e-5, "pos 3: expected 6.0, got {}", result[3]);
    assert!((result[4] - 8.0).abs() < 1e-5, "pos 4: expected 8.0, got {}", result[4]);
    assert!((result[5] - 10.0).abs() < 1e-5, "pos 5: expected 10.0, got {}", result[5]);
    assert!((result[6] - 12.0).abs() < 1e-5, "pos 6: expected 12.0, got {}", result[6]);
    assert!((result[7] - 14.0).abs() < 1e-5, "pos 7: expected 14.0, got {}", result[7]);

    // Verify candle layout - high nibbles at positions 16-31
    assert!((result[16] - 1.0).abs() < 1e-5, "pos 16: expected 1.0, got {}", result[16]);
    assert!((result[17] - 3.0).abs() < 1e-5, "pos 17: expected 3.0, got {}", result[17]);
    assert!((result[18] - 5.0).abs() < 1e-5, "pos 18: expected 5.0, got {}", result[18]);
    assert!((result[19] - 7.0).abs() < 1e-5, "pos 19: expected 7.0, got {}", result[19]);
    assert!((result[20] - 9.0).abs() < 1e-5, "pos 20: expected 9.0, got {}", result[20]);
    assert!((result[21] - 11.0).abs() < 1e-5, "pos 21: expected 11.0, got {}", result[21]);
    assert!((result[22] - 13.0).abs() < 1e-5, "pos 22: expected 13.0, got {}", result[22]);
    assert!((result[23] - 15.0).abs() < 1e-5, "pos 23: expected 15.0, got {}", result[23]);
}
...
fn test_q4_1_scale_and_min_correctness() {
    ...
    assert!((result[0] - 12.0).abs() < 0.1, "pos 0: expected 12.0, got {}", result[0]);

    // Position 16: 2.0 * 3 + 10.0 = 16.0
    assert!((result[16] - 16.0).abs() < 0.1, "pos 16: expected 16.0, got {}", result[16]);
}
...
fn test_q5_0_candle_layout_correctness() {
    ...
    assert!((result[0] - (-16.0)).abs() < 1e-5, "pos 0: expected -16.0, got {}", result[0]);

    // Position 1: 1.0 * (1 | 0 - 16) = -15.0
    assert!((result[1] - (-15.0)).abs() < 1e-5, "pos 1: expected -15.0, got {}", result[1]);

    // Position 16: 1.0 * (8 | 0 - 16) = -8.0
    assert!((result[16] - (-8.0)).abs() < 1e-5, "pos 16: expected -8.0, got {}", result[16]);

    // Position 17: 1.0 * (15 | 0 - 16) = -1.0
    assert!((result[17] - (-1.0)).abs() < 1e-5, "pos 17: expected -1.0, got {}", result[17]);
}
...
fn test_q5_0_high_bit_correctness() {
    ...
    assert!((result[16] - 0.0).abs() < 1e-5, "pos 16: expected 0.0, got {}", result[16]);
}
...
fn test_q5_1_scale_min_high_bit_correctness() {
    ...
    assert!((result[0] - 39.0).abs() < 0.1, "pos 0: expected 39.0, got {}", result[0]);

    // Position 16: 2.0 * (2 | 0) + 5.0 = 2.0 * 2 + 5 = 9.0
    assert!((result[16] - 9.0).abs() < 0.1, "pos 16: expected 9.0, got {}", result[16]);
}
...
fn test_q4_0_candle_layout_reference() {
    ...
    assert!((result[0] - (-8.0)).abs() < 1e-5, "pos 0: expected -8.0, got {}", result[0]);

    // Position 1: 1.0 * (1 - 8) = -7.0
    assert!((result[1] - (-7.0)).abs() < 1e-5, "pos 1: expected -7.0, got {}", result[1]);

    // Position 16: 1.0 * (8 - 8) = 0.0
    assert!((result[16] - 0.0).abs() < 1e-5, "pos 16: expected 0.0, got {}", result[16]);

    // Position 17: 1.0 * (15 - 8) = 7.0
    assert!((result[17] - 7.0).abs() < 1e-5, "pos 17: expected 7.0, got {}", result[17]);
}
...
fn test_q4_1_multi_block_correctness() {
    ...
    assert!((result[0] - 1.0).abs() < 1e-5, "block 0 pos 0: expected 1.0, got {}", result[0]);

    // Block 0, Position 16: 1.0 * 2 + 0.0 = 2.0
    assert!((result[16] - 2.0).abs() < 1e-5, "block 0 pos 16: expected 2.0, got {}", result[16]);

    // Block 1, Position 0: 2.0 * 3 + 10.0 = 16.0
    assert!((result[32] - 16.0).abs() < 0.1, "block 1 pos 0: expected 16.0, got {}", result[32]);

    // Block 1, Position 16: 2.0 * 4 + 10.0 = 18.0
    assert!((result[48] - 18.0).abs() < 0.1, "block 1 pos 16: expected 18.0, got {}", result[48]);
}
...
fn test_q4_1_matmul_integration() {
    ...
    let weight_matrix = TruenoMatrix::from_vec(out_dim, in_dim, weights_f32.clone()).expect("matrix creation failed");
...
fn test_q4_0_fused_matmul_qwen_dimensions() {
    ...
    for row in 0..out_dim {
        ...
        for block in 0..blocks_per_row {
            ...
            for i in 0..16 {
                data[block_start + 2 + i] = (((row + block + i) % 16) << 4 | ((row + i) % 16)) as u8;
            }
        }
    }
    ...
    eprintln!("Q4_0 Qwen dims: max diff = {} at row {}", max_diff, max_diff_idx);
    eprintln!("Fused[{}] = {}", max_diff_idx, fused_result[max_diff_idx]);
    eprintln!("Reference[{}] = {}", max_diff_idx, reference_result.as_slice()[max_diff_idx]);
