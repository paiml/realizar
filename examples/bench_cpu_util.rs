...
fn main() {
    ...
    println!("Max concurrent threads: {}", MAX_THREADS.load(Ordering::Relaxed));
    ...
    println!("\nSequential read-only: {:.0} µs (sum: {})", read_only_us, sum);
    ...
    println!("Parallel read-only:   {:.0} µs (speedup: {:.1}x)", par_read_only_us, read_only_us / par_read_only_us);
    ...
    println!("\nSingle kernel: {:.0} ns ({:.3} µs), result: {}", kernel_ns, kernel_ns / 1000.0, total);
    ...
    println!("Perfect parallel (24 cores): {:.0} µs", perfect_parallel_ns / 1000.0);
}
