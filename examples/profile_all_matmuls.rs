...
fn main() -> Result<(), RealizarError> {
    ...
    let ffn_activated: Vec<f32> = (0..intermediate_dim).map(|i| (i as f32 / intermediate_dim as f32) * 2.0 - 1.0).collect();
    ...
    fn profile_matmul(
        ...
        // Warmup
        match qtype {
            GGUF_TYPE_Q4_K => fused_q4k_parallel_matvec_into(weight_data, input, in_dim, out_dim, &mut output)?,
            GGUF_TYPE_Q6_K => fused_q6k_parallel_matvec_into(weight_data, input, in_dim, out_dim, &mut output)?,
            _ => return Ok(0.0),
        }
        ...
        println!("{:<12} {:>8.0} us  {}  {}x{}", name, us, qtype_name, out_dim, in_dim);
        Ok(us)
    }
    ...
    match &layer.qkv_weight {
        realizar::gguf::OwnedQKVWeights::Separate { q, k, v } => {
            total_us += profile_matmul("Q", &q.data, q.qtype, &hidden, q.in_dim, q.out_dim, iterations)?;
            total_us += profile_matmul("K", &k.data, k.qtype, &hidden, k.in_dim, k.out_dim, iterations)?;
            total_us += profile_matmul("V", &v.data, v.qtype, &hidden, v.in_dim, v.out_dim, iterations)?;
        }
        realizar::gguf::OwnedQKVWeights::Fused(ref w) => {
            total_us += profile_matmul("QKV", &w.data, w.qtype, &hidden, w.in_dim, w.out_dim, iterations)?;
        }
    }

    // Attn output
    let w = &layer.attn_output_weight;
    let attn_out: Vec<f32> = (0..w.in_dim).map(|i| (i as f32 / w.in_dim as f32) * 2.0 - 1.0).collect();
    total_us += profile_matmul("Attn out", &w.data, w.qtype, &attn_out, w.in_dim, w.out_dim, iterations)?;

    // FFN up
    let w = &layer.ffn_up_weight;
    total_us += profile_matmul("FFN up", &w.data, w.qtype, &hidden, w.in_dim, w.out_dim, iterations)?;

    // FFN gate
    if let Some(ref w) = layer.ffn_gate_weight {
        total_us += profile_matmul("FFN gate", &w.data, w.qtype, &hidden, w.in_dim, w.out_dim, iterations)?;
    }

    // FFN down
    let w = &layer.ffn_down_weight;
    total_us += profile_matmul("FFN down", &w.data, w.qtype, &ffn_activated, w.in_dim, w.out_dim, iterations)?;

    // Summary
    println!("\n=== Summary ===");
    println!("Per-layer matmul: {:.0} us = {:.2} ms", total_us, total_us / 1000.0);
    ...
    println!("Other (attention + overhead): {:.1} ms ({:.0}%)", other_ms, other_ms / actual_ms * 100.0);
