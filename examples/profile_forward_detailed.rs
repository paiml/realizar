...
fn main() -> Result<(), RealizarError> {
    ...
    println!("Model: {} layers, hidden={}, intermediate={}, heads={}, kv_heads={}",
             model.config.num_layers, model.config.hidden_dim,
             model.config.intermediate_dim, model.config.num_heads,
    ...
    println!("\n=== Expected vs Actual ===");
    println!("Expected (matmul + ops): {:.1} ms/token", expected_per_token_ms);
    println!("Actual:                  {:.1} ms/token", per_token_ms);
    println!("Unexplained overhead:    {:.1} ms/token ({:.0}%)",
    ...
    println!("Rayon calls per token: {} ({} layers × {} calls/layer)",
             rayon_calls_per_token, num_layers, rayon_calls_per_layer);
    println!("Estimated Rayon overhead: {:.1} ms/token", rayon_overhead_ms);
    ...
    println!("Sequential read 100MB: {:.1} ms ({:.1} GB/s)", read_time_ms, bw_gb_s);
    ...
    println!("realizar:     {:.2} ms/tok ({:.1} tok/s)", per_token_ms, tok_per_s);
    ...
    println!("If Rayon overhead ({:.1} ms) removed: {:.1} ms/tok ({:.1} tok/s)",
             rayon_overhead_ms,
