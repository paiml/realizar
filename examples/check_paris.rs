...
fn main() {
    ...
    // Find "Paris" token
    let paris_tokens: Vec<(usize, String)> = vocab.iter().enumerate()
    ...
    for word in common_words {
        ...
        if let Some(&tid) = word_tokens.first() {
            if (tid as usize) < logits.len() {
                let token_str = if (tid as usize) < vocab.len() { &vocab[tid as usize] } else { "?" };
                println!("  '{}' -> token {} '{}': logit={:.4}", word, tid, token_str, logits[tid as usize]);
            }
        }
    }
    ...
    if let Some((paris_id, _)) = paris_tokens.first() {
        ...
        println!("\n'Paris' (token {}) rank: {}/{}", paris_id, rank + 1, logits.len());
    }
}
