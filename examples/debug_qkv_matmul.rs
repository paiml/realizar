...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n=== APR QKV ===");
    println!("Weight size: {} ({} x {})", apr_qkv.len(), qkv_dim, hidden_dim);
    ...
    println!("APR QKV output first 10 (col-major): {:?}", &apr_qkv_out[..10]);
    ...
    println!("APR QKV output first 10 (row-major): {:?}", &apr_qkv_out_row[..10]);
    ...
    // Compare
    println!("\n=== Correlation ===");
    println!("APR col-major vs GGUF: {:.6}", correlation(&apr_qkv_out, &gguf_qkv));
    println!("APR row-major vs GGUF: {:.6}", correlation(&apr_qkv_out_row, &gguf_qkv));
    ...
    if correlation(&apr_qkv_out, &gguf_qkv).abs() > 0.9 {
        ...
        println!("APR: sum={:.4}, max={:.4}, min={:.4}", apr_sum, apr_max, apr_min);
    }
