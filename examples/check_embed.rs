...
fn main() {
    ...
    let tensor = model.tensors.iter().find(|t| t.name == "token_embd.weight").unwrap();
    println!("token_embd.weight:");
    println!("  dims: {:?}", tensor.dims);
    println!("  qtype: {} ({})", tensor.qtype, if tensor.qtype == 0 { "F32" } else { "other" });
    ...
    let diff: f32 = bos_emb.iter().zip(paris_emb.iter())
        .map(|(a, b)| (a - b).abs())
        .sum();
    println!("\nL1 distance between BOS and Paris embeddings: {:.4}", diff);
    ...
    println!("  non-zero: {}/{} ({:.1}%)", non_zero, embed.len(), 100.0 * non_zero as f64 / embed.len() as f64);
}
