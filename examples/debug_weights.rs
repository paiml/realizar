...
fn main() -> Result<(), RealizarError> {
    ...
    match &layer.qkv_weight {
        realizar::gguf::OwnedQKVWeights::Fused(ref w) => {
            println!("QKV: FUSED, qtype={}, in={}, out={}", w.qtype, w.in_dim, w.out_dim);
            if w.qtype == GGUF_TYPE_Q4_K { println!("  -> Q4_K"); }
            else if w.qtype == GGUF_TYPE_Q6_K { println!("  -> Q6_K"); }
        }
    ...
    // Attn output
    println!("Attn out: qtype={}, in={}, out={}",
             layer.attn_output_weight.qtype,
             layer.attn_output_weight.in_dim,
             layer.attn_output_weight.out_dim);
    if layer.attn_output_weight.qtype == GGUF_TYPE_Q4_K { println!("  -> Q4_K"); }

    // FFN up
    println!("FFN up: qtype={}, in={}, out={}",
             layer.ffn_up_weight.qtype,
             layer.ffn_up_weight.in_dim,
             layer.ffn_up_weight.out_dim);
    if layer.ffn_up_weight.qtype == GGUF_TYPE_Q4_K { println!("  -> Q4_K"); }

    // FFN gate
    if let Some(ref gate) = layer.ffn_gate_weight {
        println!("FFN gate: qtype={}, in={}, out={}", gate.qtype, gate.in_dim, gate.out_dim);
        if gate.qtype == GGUF_TYPE_Q4_K { println!("  -> Q4_K"); }
    } else {
        println!("FFN gate: NONE");
    }

    // FFN down
    println!("FFN down: qtype={}, in={}, out={}",
             layer.ffn_down_weight.qtype,
             layer.ffn_down_weight.in_dim,
             layer.ffn_down_weight.out_dim);
    if layer.ffn_down_weight.qtype == GGUF_TYPE_Q4_K { println!("  -> Q4_K"); }
    if layer.ffn_down_weight.qtype == GGUF_TYPE_Q6_K { println!("  -> Q6_K"); }
