...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let normed: Vec<f32> = embed.iter().zip(norm_weight.iter()).map(|(h, w)| h / rms * w).collect();

    println!("Test input first 5: {:?}", &normed[..5]);

    // Get Q4K bytes
    let q4k_layers = apr_model.q4k_layers.as_ref().expect("No Q4K layers");
    let q4k_bytes = q4k_layers[0].attn_output_weight.as_ref().expect("No Q4K attn_output");
    ...
    println!("F32 row vs Q4K colmajor: {:.6}", correlation(&f32_rowmajor, &q4k_colmajor));
    println!("F32 row vs Q4K rowmajor: {:.6}", correlation(&f32_rowmajor, &q4k_rowmajor));
    println!("F32 col vs Q4K colmajor: {:.6}", correlation(&f32_colmajor, &q4k_colmajor));
    println!("F32 col vs Q4K rowmajor: {:.6}", correlation(&f32_colmajor, &q4k_rowmajor));
