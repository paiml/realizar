...
fn main() {
    ...
    for i in 0..20 {
        ...
        if i < 5 {
            println!("  Pass {}: {:>6}µs ({:.1}ms)", 
                i+1, elapsed.as_micros(), elapsed.as_micros() as f64 / 1000.0);
        }
    }
    ...
    println!("\nThroughput: {:.2} tok/s (median)", 1_000_000.0 / median as f64);
    ...
    println!("All layers ({:.0}): {:.1} MB", layers, total_layer_size / 1e6);
    println!("LM head: {:.1} MB ({:.1}% of total)", lm_head_size / 1e6, 100.0 * lm_head_size / total_size);
    ...
    println!("  QKV projections:  {:.1}ms ({:.1}%)", qkv_time, 100.0 * qkv_time / theoretical_min_ms);
    println!("  Output proj:      {:.1}ms ({:.1}%)", out_time, 100.0 * out_time / theoretical_min_ms);
    println!("  FFN layers:       {:.1}ms ({:.1}%)", ffn_time, 100.0 * ffn_time / theoretical_min_ms);
    println!("  LM head:          {:.1}ms ({:.1}%)", lm_time, 100.0 * lm_time / theoretical_min_ms);
    
    let overhead_ms = actual_ms - theoretical_min_ms;
    println!("\nOverhead analysis:");
    println!("  Theoretical:  {:.1}ms", theoretical_min_ms);
    println!("  Actual:       {:.1}ms", actual_ms);
    println!("  Overhead:     {:.1}ms ({:.1}%)", overhead_ms, 100.0 * overhead_ms / actual_ms);
