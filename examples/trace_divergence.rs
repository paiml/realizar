...
fn cpu_layer_forward(hidden: &mut Vec<f32>, layer: &realizar::gguf::OwnedQuantizedLayer, 
                     num_heads: usize, num_kv_heads: usize, eps: f32) {
    ...
    let _q = fused_matmul(&normed, &q_weight.data, q_weight.qtype, q_weight.in_dim, q_weight.out_dim);
    let _k = fused_matmul(&normed, &k_weight.data, k_weight.qtype, k_weight.in_dim, k_weight.out_dim);
    let v = fused_matmul(&normed, &v_weight.data, v_weight.qtype, v_weight.in_dim, v_weight.out_dim);
    ...
    // Output projection
    let out_proj = fused_matmul(&attn_out, &layer.attn_output_weight.data, 
        layer.attn_output_weight.qtype, layer.attn_output_weight.in_dim, layer.attn_output_weight.out_dim);

    // Residual 1
    for (h, o) in hidden.iter_mut().zip(out_proj.iter()) { *h += o; }
    ...
    let gate = fused_matmul(&ffn_normed, &gate_weight.data, gate_weight.qtype, gate_weight.in_dim, gate_weight.out_dim);
    let up = fused_matmul(&ffn_normed, &layer.ffn_up_weight.data, layer.ffn_up_weight.qtype, 
        layer.ffn_up_weight.in_dim, layer.ffn_up_weight.out_dim);

    // SwiGLU
    let swiglu: Vec<f32> = gate.iter().zip(up.iter())
        .map(|(g, u)| (g / (1.0 + (-*g).exp())) * u).collect();

    // FFN down
    let ffn_down = fused_matmul(&swiglu, &layer.ffn_down_weight.data, layer.ffn_down_weight.qtype,
        layer.ffn_down_weight.in_dim, layer.ffn_down_weight.out_dim);

    // Residual 2
    for (h, f) in hidden.iter_mut().zip(ffn_down.iter()) { *h += f; }
}
...
fn main() {
    ...
    println!("Token: {}, hidden_dim: {}, num_layers: {}", token_id, hidden_dim, num_layers);

    for layer_idx in 0..num_layers {
        cpu_layer_forward(&mut hidden, &model.layers[layer_idx], num_heads, num_kv_heads, eps);
        println!("Layer {:2}: first 3 = [{:.4}, {:.4}, {:.4}], sum={:.4}", 
            layer_idx, hidden[0], hidden[1], hidden[2], hidden.iter().sum::<f32>());
    }

    // Final RMSNorm + LM head
    let normed = rms_norm(&hidden, &model.output_norm_weight, eps);
    println!("\nNormed hidden: first 3 = [{:.4}, {:.4}, {:.4}]", normed[0], normed[1], normed[2]);

    let logits = fused_matmul(&normed, &model.lm_head_weight.data, model.lm_head_weight.qtype,
