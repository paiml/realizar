...
use realizar::gguf::{MappedGGUFModel, OwnedQuantizedModel, OwnedQKVWeights, OwnedQuantizedModelCuda, OwnedQuantizedKVCache};
...
fn cpu_layer_forward(hidden: &mut Vec<f32>, model: &OwnedQuantizedModel, layer_idx: usize) {
    ...
    let normed: Vec<f32> = hidden.iter().zip(&layer.attn_norm_weight)
    ...
    // Residual 1
    for (h, o) in hidden.iter_mut().zip(&o_out) { *h += o; }
    
    // FFN norm
    let ffn_norm = layer.ffn_norm_weight.as_ref().unwrap();
    let sum_sq: f32 = hidden.iter().map(|x| x * x).sum();
    let rms = (sum_sq / hidden_dim as f32 + eps).sqrt();
    let ffn_normed: Vec<f32> = hidden.iter().zip(ffn_norm)
        .map(|(x, g)| x / rms * g).collect();
    
    // FFN
    let gate_w = layer.ffn_gate_weight.as_ref().unwrap();
    let gate = fused_matmul(&gate_w.data, &ffn_normed, gate_w.qtype, gate_w.in_dim, gate_w.out_dim);
    let up = fused_matmul(&layer.ffn_up_weight.data, &ffn_normed, layer.ffn_up_weight.qtype,
        layer.ffn_up_weight.in_dim, layer.ffn_up_weight.out_dim);
    let swiglu: Vec<f32> = gate.iter().zip(&up).map(|(g, u)| silu(*g) * u).collect();
    let down = fused_matmul(&layer.ffn_down_weight.data, &swiglu, layer.ffn_down_weight.qtype,
        layer.ffn_down_weight.in_dim, layer.ffn_down_weight.out_dim);
    
    // Residual 2
    for (h, d) in hidden.iter_mut().zip(&down) { *h += d; }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("CPU hidden before output_norm: [{:.4}, {:.4}, {:.4}, {:.4}, {:.4}], sum={:.4}, rms={:.4}", 
