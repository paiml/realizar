...
fn main() -> Result<(), RealizarError> {
    ...
    println!("\nCalls per token: {} ({} layers × 2)", calls_per_token, num_layers);
    println!("Total Q8K overhead: {:.1} µs = {:.2} ms per token", overhead_per_token_us, overhead_per_token_ms);
    ...
    println!("Q8K overhead: {:.2} ms ({:.0}% of gap)", overhead_per_token_ms, overhead_per_token_ms / gap_ms * 100.0);
