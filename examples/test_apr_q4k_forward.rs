...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(ref q4k_layers) = transformer.q4k_layers {
        ...
        if let Some(layer0) = q4k_layers.first() {
            ...
            println!("    attn_output: {:?} bytes", layer0.attn_output_weight.as_ref().map(|v| v.len()));
            println!("    ffn_gate: {:?} bytes", layer0.ffn_gate_weight.as_ref().map(|v| v.len()));
            println!("    ffn_up: {:?} bytes", layer0.ffn_up_weight.as_ref().map(|v| v.len()));
            println!("    ffn_down: {:?} bytes", layer0.ffn_down_weight.as_ref().map(|v| v.len()));
            println!("    ffn_down_q6k: {:?} bytes", layer0.ffn_down_weight_q6k.as_ref().map(|v| v.len()));
        }
    } else {
    ...
    println!("Forward completed in {:.1}ms", fwd_time.as_secs_f64() * 1000.0);
