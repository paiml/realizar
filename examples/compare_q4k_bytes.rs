...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // APR Q4K bytes
    let apr_gate = q4k_layers[0].ffn_gate_weight.as_ref().expect("no apr ffn_gate");
    ...
    let gguf_gate_tensor = gguf_layer0.ffn_gate_weight.as_ref().expect("no gguf ffn_gate");
    ...
    if apr_gate.len() == gguf_gate.len() {
        ...
        if let Some(idx) = first_mismatch_idx {
            println!("  First mismatch at byte {}: APR={}, GGUF={}", idx, apr_gate[idx], gguf_gate[idx]);
        ...
        println!("\nLength mismatch: APR={}, GGUF={}", apr_gate.len(), gguf_gate.len());
    }
    ...
    if apr_up.len() == gguf_up.len() {
        let mismatches: usize = apr_up.iter().zip(gguf_up.iter())
    ...
    let apr_down = q4k_layers[0].ffn_down_weight_q6k.as_ref().expect("no apr ffn_down_q6k");
    ...
    if apr_down.len() == gguf_down.len() {
        let mismatches: usize = apr_down.iter().zip(gguf_down.iter())
            .filter(|(&a, &b)| a != b).count();
    ...
    let gguf_result = matmul_q4k_f32_colmajor_dispatch(gguf_gate, &test_input, intermediate_dim, hidden_dim);
    println!("GGUF kernel result first 8: {:?}", &gguf_result[..8]);

    // Using APR bytes
    let apr_result = matmul_q4k_f32_colmajor_dispatch(apr_gate, &test_input, intermediate_dim, hidden_dim);
    println!("APR kernel result first 8:  {:?}", &apr_result[..8]);

    // Correlation
    let dot: f64 = gguf_result.iter().zip(apr_result.iter())
